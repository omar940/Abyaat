...
(function (root, factory) {
  ...
  const ar = {
    ...
    doneM: 'تم!', doneF: 'تمت!',
    learnDoneSub: (p) => `أتممت حفظ اليوم: ${bt(p.n)}`, learnTodaySub: (p) => `تقدّم اليوم: ${n(p.a)} من ${n(p.b)}`,
  ...
  const en = {
    ...
    learnDoneSub: (p) => `Today's memorization is done: ${bt(p.n)}`, learnTodaySub: (p) => `Today: ${n(p.a)} of ${n(p.b)}`,
    ...
    learnStart: (p) => `Start memorizing ${bt(p.n)}`, learnDoneNotice: (p) => `Today's memorization is complete (${bt(p.n)}). Well done.`, learnMore: 'Memorize one more bayt',
    learnedOf: (p) => `${n(p.n)} of ${bt(p.total)} memorized`, baytsToday: 'Bayts today', decrease: 'Decrease', increase: 'Increase',
    ...
    recitePrompt: 'Recite the bayt aloud from memory, then check.', hint: 'Hint', hideHint: 'Hide hint', revealCheck: 'Reveal to check',
    ...
    hideLast: 'Hide the last visible bayt', showNext: 'Reveal the next bayt', visibleOf: (p) => `${n(p.a)} of ${n(p.b)} visible`,
    ...
    farRowSub: (p) => `${p.poem}, ${bt(p.b)}`,
    farPrompt: (p) => `Tap a bayt you find difficult to highlight it${p.marked ? ` (${n(p.marked)})` : ''}.`, finishChapter: 'Finish chapter',
    r1: 'Forgot', r2: 'Hard', r3: 'Good', r4: 'Easy',
    rateTitle: (p) => `You highlighted ${n(p.n)} of ${bt(p.b)}`, rateNone: 'You highlighted no bayts',
    ...
    setTitle: 'Settings', perDayT: 'New bayts per day', perDayD: 'You can also change it on the New memorization screen.', perDayLbl: 'Bayts per day',
    retT: 'Target retention', retD: 'Higher means more frequent reviews.', pct: (p) => p.v + '%',
    alwaysEnT: 'Always show the English translation', alwaysEnD: 'Hidden by default; you can show it per bayt when needed.',
