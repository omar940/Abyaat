...
(function (root, factory) {
  ...
  const fresh = () => ({
    v: 1,
    settings: { perDay: 1, reviewMode: 'all', retention: 0.9, showEn: false, readerSize: 26, migrated: 2 },
    activePoem: null,
    poems: {},
    day: { date: '', newDone: 0 },
  });
  ...
  function registerPoem(p) {
    metas[p.id] = {
      id: p.id, title: p.title, fullTitle: p.fullTitle,
      chapters: p.chapters.map((c) => ({ id: c.id, title: c.title, optional: !!c.optional, len: c.bayts.length })),
    };
  }
