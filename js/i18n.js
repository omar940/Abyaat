...
(function (root, factory) {
  ...
  const ar = {
    ...
    partFinished: (p) => `أتممت «${p.title}». سيظهر في المراجعة القريبة، وبعد ${dc(p.days)} مراجعة فعلية ينتقل إلى المراجعة البعيدة.`,
    partN: (p) => `الجزء ${n(p.i)} من ${n(p.total)}`,
    /* المراجعة القريبة */
    ...
    prev: 'السابق', next: 'التالي', finishReview: 'أتممت المراجعة',
    ...
    farLede: (p) => 'راجع اليوم: ' + (p.n === 1 ? 'جزءًا واحدًا' : n(p.n) + (p.n === 2 ? ' جزأين' : ' أجزاء')), farNone: 'لا شيء مستحق اليوم.', upcoming: 'قادمة',
    farRowSub: (p) => `${p.poem}، ${bt(p.b)}`,
    farPrompt: (p) => `المس مرّتين البيت الذي يصعب عليك للتأشير عليه${p.marked ? ` (${n(p.marked)})` : ''}.`, finishChapter: 'أتممت الفصل',
    ...
    farDoneNext: (p) => `المراجعة القادمة لهذا الفصل ${p.when}.`,
    /* المكتب */
    ...
    about: 'أبيات، الإصدار ١. المراجعة المتباعدة بخوارزمية FSRS، ببطاقة لكل ٨ إلى ١٢ بيتًا من الفصل.\nالخطوط: الأميري وIBM Plex Sans Arabic (رخصة OFL).',
    /* رسائل */
  ...
  const en = {
    ...
    partFinished: (p) => `You finished “${p.title}”. It will appear in new review, and after ${dc(p.days)} of actual review it moves to old review.`,
    partN: (p) => `Part ${n(p.i)} of ${n(p.total)}`,
    ...
    prev: 'Previous', next: 'Next', finishReview: 'Finish review',
    ...
    farLede: (p) => `Review today: ${n(p.n)} part${p.n === 1 ? '' : 's'}`, farNone: 'Nothing due today.', upcoming: 'Upcoming',
    farRowSub: (p) => `${p.poem}, ${bt(p.b)}`,
    farPrompt: (p) => `Double-tap a verse you find difficult to highlight it${p.marked ? ` (${n(p.marked)})` : ''}.`, finishChapter: 'Finish chapter',
    ...
    farDoneNext: (p) => `This chapter’s next review is ${p.when}.`,
